...
ggplot2::theme_set(mskRvis::theme_biostat()); mskRvis::set_msk_palette()
...
source('./scripts/Mel - SelectCohort.R') # Add the SelectCohort Function

# Load local functions
path <- '/Volumes/OutcomesShared/Outcomes/Andrew/Amplio/Functions/porwals/'
...
# Load Data
AllData <- readRDS(file = here_data("Results/df_main.Rds"))

# Initialize the dataframe where the final results will be stored
userResults <- NULL
# Setting last surgery date
lastSurgeryDate <- max(AllData$surgDate) # ~1 month behind query pull date
...
```{r Proportion Positive SLND then LND }
...
# Exporting all model results (adjusted and unadjusted) to CSV file
write.table(userResults,
            file = here_data('Results/SurgeonRates_full.csv'),
            sep = ",",
            row.names = F)
...
write.table(userResults_trim,
            file = here_data('Results/SurgeonRates.csv'),
            sep = ",",
            row.names = F)

# Save out workspace
save.image(here_data('Results/SurgeonRates.RData'))

# upload_data("Mel", userResults_trim)
